...
namespace Windawesome.Widgets
{
  ...
  {
    // (hWnd, uID) -> (TrayIcon, PictureBox, ToolTip)
    private readonly Dictionary<Tuple<int, uint>, Tuple<SystemTray.TrayIcon, PictureBox, ToolTip>> _icons;

    private Bar _bar;
    private int _left, _right;
    private bool _isLeft;
    private bool _showFullSystemTray;
    ...
    public bool ShowFullSystemTray 
    {
      get { return _showFullSystemTray; }
      set { _showFullSystemTray = SystemAndProcessInformation.isAtLeast7 && value; }
    }

    public SystemTrayWidget()
    {
      _icons = new Dictionary<Tuple<int, uint>, Tuple<SystemTray.TrayIcon, PictureBox, ToolTip>>(10);
    ...
    public SystemTrayWidget(bool showFullSystemTray = false)
      : this()
    {
      ShowFullSystemTray = showFullSystemTray;
    }
    ...
    private void GetIcons()
    {
      ...
      foreach (var icon in SystemTray.GetButtons(SystemAndProcessInformation.trayHandle))
      {
        ...
        _icons[Tuple.Create((int)icon.hWnd, icon.id)] = Tuple.Create(icon, pictureBox, toolTip);
      }
    ...
    private void OnIconAdded(NativeMethods.NOTIFYICONDATA iconData, Tuple<int, uint> tuple)
    {
      if (!_icons.ContainsKey(tuple))
      {
        ...
        {
          RepositionControls(_left, _right);
          _bar.DoWidgetControlsChanged(this, Enumerable.Empty<PictureBox>(), new[] { pictureBox });
        }
      }
    }
    ...
    private void OnIconModified(NativeMethods.NOTIFYICONDATA iconData, Tuple<int, uint> tuple)
    {
      ...
      if (_icons.TryGetValue(tuple, out trayIcon) && UpdateIconData(trayIcon, iconData))
      {
        RepositionControls(_left, _right);
        if (TrayIconVisible(trayIcon.Item1))
        {
          _bar.DoWidgetControlsChanged(this, Enumerable.Empty<PictureBox>(), new[] { trayIcon.Item2 });
        }
        else
        {
          _bar.DoWidgetControlsChanged(this, new[] { trayIcon.Item2 }, Enumerable.Empty<PictureBox>());
        }
      }
    }
    ...
    private void OnIconRemoved(Tuple<int, uint> tuple)
    {
      ...
      if (_icons.TryGetValue(tuple, out trayIcon))
      {
        _icons.Remove(tuple);
    ...
    {
      return _icons.Values.Where(t => TrayIconVisible(t.Item1)).Select(t => t.Item2);
    }
    ...
    public void RepositionControls(int left, int right)
    {
      ...
      if (_isLeft)
      {
        ...
        {
          pictureBox.Size = new Size(_bar.GetBarHeight(), _bar.GetBarHeight());
          pictureBox.Location = new Point(left, 0);
          left += _bar.GetBarHeight();
        }
        this._right = left;
      }
      else
    ...
    void IWidget.Refresh()
    {
      ...
      GetIcons();
      RepositionControls(_left, _right);
      _bar.DoWidgetControlsChanged(this, oldIcons, GetPictureBoxes());
    }
